...
async function getPrNumber({ eventName, issueEvent, owner, repo, label, placeholderFiles }: CreatePROptions) {
  ...
  if (isPr) {
    ...
    const connectedPrNumber = await github.getConnectedPr(owner, repo, issue.number);
...
export async function getPrInformation(label: string, placeholderFiles: Record<string, string>) {
  ...
  const { pr, issue } = await getPrNumber({eventName, issueEvent, owner, repo, branch, label, placeholderFiles});
  const { prompt, images } = await getPromptAndImages(owner, repo, issue, pr, branch);
