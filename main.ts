...
const SYSTEM_PROMPT = await Deno.readTextFile(join(new URL(".", import.meta.url).pathname, "./prompts/ui-gen.md"));

(async function main() {
  const res = await getPrInformation(LABEL, { PREVIEW_FILE_PATH : PLACEHOLDER_CODE });
  if (!res) {
    return;
  }
  ...
  let { prompt } = res;
  const currentCode = await github.getFileContent(owner, repo, branch, PREVIEW_FILE_PATH);
